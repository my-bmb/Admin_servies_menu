# admin_app.py - COMPLETE WITH SAME TABLES AS CUSTOMER APP
import os
import sys
import logging
        ...
        if request.method == 'POST':
            ...
            if 'category_photo' in request.files:
                ...
                if file and file.filename:
                    if cloudinary_configured:
                        try:
                            # Delete old image from Cloudinary (using public_id pattern)
                            if category.get('category_photo'):
                                try:
                                    # Extract public_id from URL
                                    public_id = category['category_photo'].split('/')[-1].split('.')[0]
                                    cloudinary.uploader.destroy(f"{SERVICE_CATEGORIES_FOLDER}/{public_id}")
                                except:
                                    pass
...
def delete_service_category(category_id):
    try:
        ...
        if category.get('category_photo') and cloudinary_configured:
            try:
                public_id = category['category_photo'].split('/')[-1].split('.')[0]
                cloudinary.uploader.destroy(f"{SERVICE_CATEGORIES_FOLDER}/{public_id}")
            except:
                    ...
                    if cloudinary_configured:
                        try:
                            ...
                            if service.get('photo'):
                                try:
                                    public_id = service['photo'].split('/')[-1].split('.')[0]
                                    cloudinary.uploader.destroy(f"{SERVICES_FOLDER}/{public_id}")
                                except:
                                    pass
...
def delete_service(id):
    try:
        ...
        if services_list:
            ...
            # Delete image from Cloudinary
            if service.get('photo') and cloudinary_configured:
                try:
                    public_id = service['photo'].split('/')[-1].split('.')[0]
                    cloudinary.uploader.destroy(f"{SERVICES_FOLDER}/{public_id}")
                except:
                    ...
                    if cloudinary_configured:
                        try:
                            if category.get('category_photo'):
                                try:
                                    public_id = category['category_photo'].split('/')[-1].split('.')[0]
                                    cloudinary.uploader.destroy(f"{MENU_CATEGORIES_FOLDER}/{public_id}")
                                except:
                                    pass
        ...
        if request.method == 'POST':
            ...
            if 'photo' in request.files:
                ...
                if file and file.filename:
                    if cloudinary_configured:
                        try:
                            if menu_item.get('photo'):
                                try:
                                    public_id = menu_item['photo'].split('/')[-1].split('.')[0]
                                    cloudinary.uploader.destroy(f"{MENU_FOLDER}/{public_id}")
                                except:
                                    pass
...
def delete_menu_item(id):
    try:
        ...
        if menu_items_list:
            ...
            if menu_item.get('photo') and cloudinary_configured:
                try:
                    public_id = menu_item['photo'].split('/')[-1].split('.')[0]
                    cloudinary.uploader.destroy(f"{MENU_FOLDER}/{public_id}")
                except:
