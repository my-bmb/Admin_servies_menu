...
def update_service_position():
    """Update service position via AJAX - IMPROVED VERSION"""
    try:
        ...
        if not all_services:
            return jsonify({'success': False, 'error': 'No services found'})
        ...
        all_services.pop(target_index)
        
        # Insert at new position
        insert_index = new_position - 1
        if insert_index > len(all_services):
            insert_index = len(all_services)
        if insert_index < 0:
            insert_index = 0
        all_services.insert(insert_index, target_service)
        
        # Update all positions
        for idx, service in enumerate(all_services, start=1):
            current_position = service.get('position', 0)
            if current_position != idx:
                result = supabase_execute('services', 'update',
                                        data={'position': idx},
                                        conditions={'id': service['id']},
                                        use_admin=True)
                if not result:
                    print(f"Warning: Failed to update position for service {service['id']}")
        
        return jsonify({'success': True, 'message': 'Position updated successfully'})
        
    except Exception as e:
        print(f"Error updating service position: {str(e)}")
...
def update_menu_position():
    """Update menu position via AJAX - IMPROVED VERSION"""
    try:
        data = request.get_json()
        menu_id = data['id']
        new_position = int(data['position'])
        ...
        if not all_menu_items:
            return jsonify({'success': False, 'error': 'No menu items found'})
        ...
        target_item = None
        target_index = -1
        for idx, m in enumerate(all_menu_items):
            if m['id'] == menu_id:
                target_item = m
                target_index = idx
                break
        ...
        # Remove target from list
        all_menu_items.pop(target_index)
        
        # Insert at new position
        insert_index = new_position - 1
        if insert_index > len(all_menu_items):
            insert_index = len(all_menu_items)
        if insert_index < 0:
            insert_index = 0
        all_menu_items.insert(insert_index, target_item)
        
        # Update all positions
        for idx, menu_item in enumerate(all_menu_items, start=1):
            current_position = menu_item.get('position', 0)
            if current_position != idx:
                result = supabase_execute('menu', 'update',
                                        data={'position': idx},
                                        conditions={'id': menu_item['id']},
                                        use_admin=True)
                if not result:
                    print(f"Warning: Failed to update position for menu item {menu_item['id']}")
        
        return jsonify({'success': True, 'message': 'Position updated successfully'})
        
    except Exception as e:
        print(f"Error updating menu position: {str(e)}")
        return jsonify({'success': False, 'error': str(e)})
