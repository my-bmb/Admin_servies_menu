# admin_app.py - UPDATED WITH CATEGORY MANAGEMENT
import os
import sys
import logging
...
def service_categories():
    ...
    try:
        ...
        if status_filter:
            categories = [c for c in categories if c.get('status') == status_filter]
        ...
        return render_template('admin/service_categories.html', categories=categories, search=search, status_filter=status_filter)
    except Exception as e:
        flash(f'Error loading categories: {str(e)}', 'error')
        return render_template('admin/service_categories.html', categories=[], search='', status_filter='')
...
def delete_service_category(category_id):
    ...
    try:
        ...
        if services and len(services) > 0:
            flash(f'Cannot delete category with {len(services)} services. Delete services first or move them to another category.', 'error')
            return redirect(url_for('service_categories'))
...
def services():
    ...
    try:
        ...
        return render_template('admin/services.html', services=[], categories=[], search='', status_filter='', category_filter='')

@app.route('/admin/services/add', methods=['GET', 'POST'])
@login_required
def add_service():
    """Add new service with category"""
    if request.method == 'POST':
        try:
            name = request.form.get('name', '').strip()
...
def menu_categories():
    ...
    try:
        ...
        return render_template('admin/menu_categories.html', categories=categories, search=search, status_filter=status_filter)
    except Exception as e:
        flash(f'Error loading categories: {str(e)}', 'error')
        return render_template('admin/menu_categories.html', categories=[], search='', status_filter='')
...
def delete_menu_category(category_id):
    ...
    try:
        ...
        if items and len(items) > 0:
            flash(f'Cannot delete category with {len(items)} items. Delete items first or move them to another category.', 'error')
            return redirect(url_for('menu_categories'))
...
@login_required
def menu_items():
    """List all menu items with category info"""
    try:
        search = request.args.get('search', '')
        status_filter = request.args.get('status', '')
        category_filter = request.args.get('category', '')
        ...
        if search:
            menu_items_list = [m for m in menu_items_list if search.lower() in m.get('name', '').lower()]
        
        if status_filter:
            menu_items_list = [m for m in menu_items_list if m.get('status') == status_filter]
        
        if category_filter:
            menu_items_list = [m for m in menu_items_list if m.get('category_id') == int(category_filter)]
        ...
        return render_template('admin/menu_items.html', menu_items=menu_items_list, 
                               categories=categories, search=search, 
                               status_filter=status_filter, category_filter=category_filter)
    except Exception as e:
        flash(f'Error loading menu items: {str(e)}', 'error')
        return render_template('admin/menu_items.html', menu_items=[], categories=[], search='', status_filter='', category_filter='')

@app.route('/admin/menu-items/add', methods=['GET', 'POST'])
@login_required
def add_menu_item():
    """Add new menu item with category"""
    if request.method == 'POST':
        try:
            name = request.form.get('name', '').strip()
            category_id = request.form.get('category_id')
...
# ============================================
# ✅ DATA EXPORT APIs (UPDATED FOR CATEGORIES)
# ============================================
...
@login_required
def edit_positions():
    """Edit positions of categories, services, and menu items"""
    try:
        service_categories = supabase_execute('service_categories', 'select')
        service_categories = sorted(service_categories, key=lambda x: x.get('position', 0))
        
        menu_categories = supabase_execute('menu_categories', 'select')
        menu_categories = sorted(menu_categories, key=lambda x: x.get('position', 0))
...
@login_required
def update_position():
    """Update position for any entity (category, service, menu item)"""
    try:
        ...
        if table_name in ['services', 'menu_items']:
            filter_field = 'category_id' if table_name == 'services' else 'category_id'
