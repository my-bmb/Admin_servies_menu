...
def safe_int_conversion(value):
    """Safely convert a value to integer, handling float strings like '9.0'"""
    if value is None:
        return None
    try:
        ...
        if not str_value:
            return None
        # Handle decimal values (e.g., "4.0", "9.0")
        if '.' in str_value:
...
def add_service_category():
    ...
    if request.method == 'POST':
        try:
            ...
            # ✅ Fix: Convert collection_id safely
...
def add_service():
    ...
    if request.method == 'POST':
        try:
            ...
            price = safe_float_conversion(request.form.get('price', 0))
            discount = safe_float_conversion(request.form.get('discount', 0))
            ...
            # ✅ Fix: Convert category_id safely
            ...
            if category_services:
                positions = [s.get('position', 0) for s in category_services if s.get('position') is not None]
                if positions:
...
@login_required
def add_goods_item():
    """Add new goods item under a category"""
    if request.method == 'POST':
        try:
            name = request.form.get('name', '').strip()
            category_id_raw = request.form.get('category_id')
            ...
            # ✅ FIX: Get max position - ensure integer
            try:
                ...
                if category_items and len(category_items) > 0:
                    # Get all positions and filter out None values
                    positions = []
                    for item in category_items:
                        ...
                        if pos is not None:
                            try:
                                ...
                                positions.append(int(float(pos)))
                            except:
                                pass
                    if positions:
                        max_position = max(positions)
            ...
            # Ensure position is integer and not float
            ...
            print(f"Inserting item data: {item_data}")
            
            result = supabase_execute('goods_items', 'insert', data=item_data, use_admin=True)
