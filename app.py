# admin_app.py - COMPLETE FIXED VERSION FOR RENDER
...
def render_diagnostics():
    ...
    if python_version.major != 3:
        ...
        warnings.append("Python 3.12+ may need specific psycopg version")
    else:
    ...
    try:
        ...
        checks_passed.append("Flask installed")
    except ImportError:
        print("   ❌ FAILURE: Flask not installed")
        checks_failed.append("Flask missing")
    print()
    ...
    # 5️⃣ DATABASE URL VALIDATION
    print("5️⃣ DATABASE URL VALIDATION")
    ...
    if db_url:
        ...
        if 'postgres://' in db_url:
            ...
            print(f"   ✔ SUCCESS: Database URL scheme is valid")
...
# ✅ FIXED: SIMPLE PSYCOPG IMPORTS (NO POOL MODULE)
import psycopg
...
# ✅ DATABASE CONFIGURATION - FIXED FOR PYTHON 3.13
...
def get_db_connection():
    """Get database connection - Simple implementation"""
    try:
...
def execute_query(query, params=None, fetch_one=False, fetch_all=False, commit=True):
    """Execute database query - SIMPLE VERSION"""
    conn = None
    try:
...
if __name__ == '__main__':
    ...
    if os.environ.get('RENDER'):
        print(f"\n🚀 Starting Admin Dashboard on port {port}")
