...
def safe_int_conversion(value):
    ...
    if value is None:
        return None
    try:
        # Convert to string first, then handle decimal
...
def add_goods_item():
    ...
    if request.method == 'POST':
        try:
            ...
            # ✅ Fix: Convert category_id safely - handle "4.0" to 4
            category_id = safe_int_conversion(category_id_raw)
            if category_id is None:
                flash('Invalid category selected', 'error')
                return redirect(url_for('add_goods_item'))
            ...
            try:
                ...
                max_position = 0
                if category_items:
                    positions = [i.get('position', 0) for i in category_items if i.get('position') is not None]
                    if positions:
                        max_position = max(positions)
                position = max_position + 1
            except Exception as e:
            ...
            print(f"Inserting item data: {item_data}")  # Debug print
