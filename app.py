# app.py - Complete Service & Menu Management System
# Service Hierarchy: Collection → Category → Service
# Menu Hierarchy: Collection → Category → Item
import os
import sys
import logging
...
MENU_COLLECTIONS_FOLDER = "menu_collections"
MENU_CATEGORIES_FOLDER = "menu_categories"
MENU_ITEMS_FOLDER = "menu_items"
...
def dashboard():
    ...
    try:
        ...
        # Menu counts
        menu_collections = supabase_execute('menu_collections', 'select')
        ...
        menu_collections_count = len(menu_collections) if menu_collections else 0
        ...
        active_menu_collections = len([c for c in menu_collections if c.get('status') == 'active']) if menu_collections else 0
        active_menu_categories = len([c for c in menu_categories if c.get('status') == 'active']) if menu_categories else 0
        active_menu_items = len([i for i in menu_items if i.get('status') == 'active']) if menu_items else 0
        
        return render_template('admin/dashboard.html',
                             service_collections_count=service_collections_count,
                             service_categories_count=service_categories_count,
                             services_count=services_count,
                             menu_collections_count=menu_collections_count,
                             menu_categories_count=menu_categories_count,
                             menu_items_count=menu_items_count,
                             active_service_collections=active_service_collections,
                             active_service_categories=active_service_categories,
                             active_services=active_services,
                             active_menu_collections=active_menu_collections,
                             active_menu_categories=active_menu_categories,
...
# ============================================
# ✅ MENU COLLECTIONS MANAGEMENT
# ============================================

@app.route('/admin/menu-collections')
@login_required
def menu_collections():
    """List all menu collections"""
    try:
        search = request.args.get('search', '')
        status_filter = request.args.get('status', '')
        
        collections_list = supabase_execute('menu_collections', 'select')
        ...
        # Get category count for each collection
        for collection in collections_list:
            categories = supabase_execute('menu_categories', 'select', conditions={'collection_id': collection['id']})
            collection['category_count'] = len(categories) if categories else 0
        
        return render_template('admin/menu_collections.html', collections=collections_list, 
                             search=search, status_filter=status_filter)
    except Exception as e:
        flash(f'Error loading menu collections: {str(e)}', 'error')
        return render_template('admin/menu_collections.html', collections=[], search='', status_filter='')

@app.route('/admin/menu-collections/add', methods=['GET', 'POST'])
@login_required
def add_menu_collection():
    """Add new menu collection"""
    if request.method == 'POST':
        try:
            name = request.form.get('name', '').strip()
            description = request.form.get('description', '').strip()
            ...
            if not name:
                flash('Collection name is required', 'error')
                return redirect(url_for('add_menu_collection'))
                ...
                if file and file.filename:
                    if not cloudinary_configured:
                        ...
                        try:
                            upload_result = cloudinary.uploader.upload(
                                file,
                                folder=MENU_COLLECTIONS_FOLDER,
                                public_id=f"menu_collection_{name.lower().replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                                overwrite=True,
                                transformation=[
                                    {'width': 400, 'height': 400, 'crop': 'fill'},
                                    {'quality': 'auto', 'fetch_format': 'auto'}
            ...
            if position == 0:
                all_collections = supabase_execute('menu_collections', 'select')
                if all_collections:
                    positions = [c.get('position', 0) for c in all_collections]
                    position = max(positions) + 1 if positions else 1
            ...
            supabase_execute('menu_collections', 'insert', data=collection_data, use_admin=True)
            
            flash(f'Menu collection "{name}" added successfully!', 'success')
            return redirect(url_for('menu_collections'))
            
        except Exception as e:
            flash(f'Error adding menu collection: {str(e)}', 'error')
    
    return render_template('admin/add_edit_menu_collection.html', collection=None)

@app.route('/admin/menu-collections/edit/<int:collection_id>', methods=['GET', 'POST'])
@login_required
def edit_menu_collection(collection_id):
    """Edit existing menu collection"""
    try:
        collections_list = supabase_execute('menu_collections', 'select', conditions={'id': collection_id})
        ...
        if request.method == 'POST':
            ...
            if not name:
                flash('Collection name is required', 'error')
                return redirect(url_for('edit_menu_collection', collection_id=collection_id))
            ...
            supabase_execute('menu_collections', 'update', data=update_data, 
                           conditions={'id': collection_id}, use_admin=True)
            
            flash(f'Menu collection "{name}" updated successfully!', 'success')
            return redirect(url_for('menu_collections'))
        
        return render_template('admin/add_edit_menu_collection.html', collection=collection)
        
    except Exception as e:
        flash(f'Error loading collection: {str(e)}', 'error')
        return redirect(url_for('menu_collections'))

@app.route('/admin/menu-collections/delete/<int:collection_id>', methods=['POST'])
@login_required
def delete_menu_collection(collection_id):
    ...
    try:
        ...
        # Check if collection has categories
        categories = supabase_execute('menu_categories', 'select', conditions={'collection_id': collection_id})
        ...
        supabase_execute('menu_collections', 'delete', conditions={'id': collection_id}, use_admin=True)
        
        flash(f'Menu collection "{collection["name"]}" deleted successfully!', 'success')
...
@app.route('/admin/menu-collections/toggle-status/<int:collection_id>')
@login_required
def toggle_menu_collection_status(collection_id):
    """Toggle menu collection status"""
    try:
        collections_list = supabase_execute('menu_collections', 'select', conditions={'id': collection_id})
        ...
        supabase_execute('menu_collections', 'update', 
                        data={'status': new_status}, 
                        conditions={'id': collection_id}, 
                        use_admin=True)
        
        status_text = "activated" if new_status == 'active' else "deactivated"
        flash(f'Menu collection "{collection["name"]}" {status_text} successfully!', 'success')
...
@login_required
def menu_categories():
    """List all menu categories with collection info"""
    try:
        search = request.args.get('search', '')
        status_filter = request.args.get('status', '')
        collection_filter = request.args.get('collection', '')
        
        categories_list = supabase_execute('menu_categories', 'select')
        ...
        for category in categories_list:
            items = supabase_execute('menu_items', 'select', conditions={'category_id': category['id']})
            category['item_count'] = len(items) if items else 0
        
        return render_template('admin/menu_categories.html', categories=categories_list, 
                               collections=collections_list, search=search, 
                               status_filter=status_filter, collection_filter=collection_filter)
    except Exception as e:
        flash(f'Error loading menu categories: {str(e)}', 'error')
        return render_template('admin/menu_categories.html', categories=[], collections=[], 
                             search='', status_filter='', collection_filter='')

@app.route('/admin/menu-categories/add', methods=['GET', 'POST'])
@login_required
def add_menu_category():
    """Add new menu category under a collection"""
    if request.method == 'POST':
        try:
            name = request.form.get('name', '').strip()
            collection_id = request.form.get('collection_id')
                    ...
                    if not cloudinary_configured:
                        ...
                        try:
                            upload_result = cloudinary.uploader.upload(
                                ...
                                public_id=f"menu_category_{name.lower().replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                                overwrite=True,
                                transformation=[
                                    {'width': 400, 'height': 400, 'crop': 'fill'},
                                    {'quality': 'auto', 'fetch_format': 'auto'}
            ...
            # Get max position for this collection
            collection_categories = supabase_execute('menu_categories', 'select', conditions={'collection_id': collection_id})
            if position == 0:
                if collection_categories:
                    positions = [c.get('position', 0) for c in collection_categories]
                    position = max(positions) + 1 if positions else 1
            ...
            flash(f'Error adding menu category: {str(e)}', 'error')
    
    # Get collections for dropdown
    collections_list = supabase_execute('menu_collections', 'select')
    collections_list = sorted(collections_list, key=lambda x: x.get('position', 0))
    
    return render_template('admin/add_edit_menu_category.html', category=None, collections=collections_list)

@app.route('/admin/menu-categories/edit/<int:category_id>', methods=['GET', 'POST'])
@login_required
def edit_menu_category(category_id):
    """Edit existing menu category"""
    try:
        categories_list = supabase_execute('menu_categories', 'select', conditions={'id': category_id})
        ...
        return render_template('admin/add_edit_menu_category.html', category=category, collections=collections_list)
...
@login_required
def menu_items():
    """List all menu items with category and collection info"""
    try:
        search = request.args.get('search', '')
        status_filter = request.args.get('status', '')
        category_filter = request.args.get('category', '')
        ...
        return render_template('admin/menu_items.html', items=items_list, 
                               categories=categories_list, collections=collections_list,
                               search=search, status_filter=status_filter, 
                               category_filter=category_filter, collection_filter=collection_filter)
    except Exception as e:
        flash(f'Error loading menu items: {str(e)}', 'error')
        return render_template('admin/menu_items.html', items=[], categories=[], collections=[],
                             search='', status_filter='', category_filter='', collection_filter='')

@app.route('/admin/menu-items/add', methods=['GET', 'POST'])
@login_required
def add_menu_item():
    """Add new menu item under a category"""
    if request.method == 'POST':
        try:
            name = request.form.get('name', '').strip()
            category_id = request.form.get('category_id')
                    ...
                    if not cloudinary_configured:
                        ...
                        try:
                            upload_result = cloudinary.uploader.upload(
                                ...
                                public_id=f"menu_item_{name.lower().replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                                overwrite=True,
                                transformation=[
                                    {'width': 800, 'height': 600, 'crop': 'fill'},
                                    {'quality': 'auto', 'fetch_format': 'auto'}
    ...
    return render_template('admin/add_edit_menu_item.html', item=None, categories=categories_list)
...
def edit_menu_item(id):
    ...
    try:
        items_list = supabase_execute('menu_items', 'select', conditions={'id': id})
        ...
        return render_template('admin/add_edit_menu_item.html', item=item, categories=categories_list)
...
def toggle_menu_item_status(id):
    ...
    try:
        ...
        status_text = "activated" if new_status == 'active' else "deactivated"
        flash(f'Menu item "{item["name"]}" {status_text} successfully!', 'success')
...
@app.route('/api/menu/full')
def export_full_menu():
    """Public API to fetch full menu with hierarchy: Collections → Categories → Items"""
    try:
        # Get active menu collections
        collections = supabase_execute('menu_collections', 'select', conditions={'status': 'active'})
        collections = sorted(collections, key=lambda x: x.get('position', 0))
...
def edit_positions():
    ...
    try:
        ...
        # Menu side
        menu_collections_list = supabase_execute('menu_collections', 'select')
        menu_collections_list = sorted(menu_collections_list, key=lambda x: x.get('position', 0))
        
        menu_categories_list = supabase_execute('menu_categories', 'select')
        menu_categories_list = sorted(menu_categories_list, key=lambda x: (x.get('collection_id', 0), x.get('position', 0)))
        ...
        menu_collections_dict = {c['id']: c['name'] for c in menu_collections_list}
        for cat in menu_categories_list:
            cat['collection_name'] = menu_collections_dict.get(cat.get('collection_id'), 'Unknown')
        
        menu_categories_dict = {c['id']: c['name'] for c in menu_categories_list}
        for item in menu_items_list:
            item['category_name'] = menu_categories_dict.get(item.get('category_id'), 'Unknown')
        ...
        return render_template('admin/edit_positions.html', 
                             ...
                             menu_collections=menu_collections_list,
...
def update_position():
    ...
    try:
        ...
        elif entity_type == 'menu_collection':
            table_name = 'menu_collections'
...
def health_check():
    try:
        return jsonify({
            'status': 'healthy',
            'service': 'Service & Menu Management System',
            'hierarchy': {
                'services': 'Collection → Category → Service',
                'menu': 'Collection → Category → Item'
            },
...
CREATE TABLE IF NOT EXISTS menu_categories (
    id SERIAL PRIMARY KEY,
    collection_id INTEGER REFERENCES menu_collections(id) ON DELETE CASCADE,
    name VARCHAR(100) NOT NULL,
    description TEXT,
    category_photo VARCHAR(500),
    status VARCHAR(20) DEFAULT 'active',
...
CREATE INDEX IF NOT EXISTS idx_menu_categories_collection_id ON menu_categories(collection_id);
...
CREATE POLICY "Public can read menu_collections" ON menu_collections FOR SELECT USING (true);
CREATE POLICY "Public can read menu_categories" ON menu_categories FOR SELECT USING (true);
...
if __name__ == '__main__':
    ...
    print(f"\n🚀 Starting Service & Menu Management System")
    print(f"📊 Service Hierarchy: Collection → Category → Service")
    print(f"📊 Menu Hierarchy: Collection → Category → Item")
    ...
    print(f"   Menu: http://localhost:{port}/api/menu/full")
    print("\n" + "="*60)
